...
class VRPModel(nn.Module):
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, state.load,state.length, ninf_mask=state.ninf_mask)
...
class VRP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, load, length, ninf_mask):
        ...
        #######################################################
        input_cat = torch.cat((encoded_last_node, load[:, :, None],length[:, :, None]), dim=2)
